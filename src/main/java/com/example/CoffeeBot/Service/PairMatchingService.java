...
public class PairMatchingService {
    ...
    private List<CoffeeMeeting> createBasicPairs(List<Subscriber> shuffledUsers, Map<Long, Set<Long>> recentMeetings) {
        ...
        for (int i = 0; i < shuffledUsers.size(); i += 2) {
            if (i+1 < shuffledUsers.size()) {
                ...
                Subscriber subscriber2 = shuffledUsers.get(i+1);

                if(haveSubscribersMetRecently(subscriber1, subscriber2, recentMeetings)) {
                    Subscriber alternative = findAlternativePartner(subscriber1, shuffledUsers, i+2, recentMeetings);
                    if (alternative != null) {
                        int altIndex = shuffledUsers.indexOf(alternative);
                        Collections.swap(shuffledUsers, i+1, altIndex);
                    }
                }
    ...
    public Subscriber findAlternativePartner(Subscriber subscriber, List<Subscriber> subscribers, int startIndex, Map<Long, Set<Long>> recentMeetings) {
        for (int i = startIndex; i < subscribers.size(); i++) {
            Subscriber candidate = subscribers.get(i);
     ...
     */
    private boolean haveSubscribersMetRecently(Subscriber subscriber1, Subscriber subscriber2, Map<Long, Set<Long>> recentMeetings) {
    ...
    public List<Subscriber> getActiveSubscribers() {
        return subscriberRepository.findByIsActive();
    }

    public boolean isEvenNumberOfSubscribers(List<Subscriber> subscribers) {
        return subscribers.size() % 2 == 0;
    }
}
