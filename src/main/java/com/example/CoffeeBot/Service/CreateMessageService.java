...
@Service
@Slf4j
// если класс называется сервисом, то обычно это подразумевает, что в нем содержится бизнес-логика приложения
// этот класс больше похож на какой-то сборщик текстов сообщений
// у тебя в классах-хэндлерах больше бизнес-логики. А хэндлер это просто обработчик, который триггерится при каком-то событии (пришло сообщение например)
// и уже вызывает метод сервиса, в котором должна быть логика обработки
